...
class Architect(object):
    ...
    def step(self,arg, input_train, target_train, input_valid, target_valid,eta=None, network_optimizer=None, unrolled=False):
        for optimizer in self.optimizers:
            optimizer.zero_grad()
        if unrolled:
                loss = self._backward_step_unrolled(input_train, target_train, input_valid, target_valid, eta, network_optimizer)
        else:
                loss, loss_latency, loss_flops = self._backward_step(arg,input_valid, target_valid)
        # 构造回归损失函数
    ...
    def _backward_step(self,arg, input_valid, target_valid):
        loss = self.model._loss(arg,input_valid, target_valid)
        loss_latency = 0
        loss_flops = 0
        self.latency_supernet = 0
